...
[role="lead"]
I sistemi ASA r2 utilizzano le zone di disponibilità dello storage invece degli aggregati, quindi alcuni comandi e parametri della CLI di ONTAP non sono supportati. Puoi anche usare la CLI per configurare un cluster se la preferisci a System Manager.
...
Salvo diversa indicazione, tutti gli altri comandi nel link:https://docs.netapp.com/us-en/ontap-cli/["Riferimento comando ONTAP"] sono supportati per i sistemi ASA r2.

I seguenti comandi e set di comandi non sono supportati su ASA r2:
...
Si consiglia che tu link:../install-setup/initialize-ontap-cluster.html["Utilizza System Manager per configurare il cluster ONTAP ASA R2"]. System Manager offre un flusso di lavoro guidato rapido e semplice per avviare il tuo cluster. Tuttavia, se sei abituato a lavorare con i comandi ONTAP, puoi usare facoltativamente l'interfaccia a riga di comando (CLI) di ONTAP per la configurazione del cluster. La configurazione del cluster tramite CLI non offre opzioni o vantaggi aggiuntivi rispetto a System Manager.

Durante la configurazione del cluster, ONTAP crea la tua VM di storage dati predefinita, crea un'unità di storage iniziale e rileva automaticamente le tue LIF di dati. Facoltativamente, puoi abilitare il Domain Name System (DNS) per risolvere i nomi host, impostare il cluster per usare il Network Time Protocol (NTP) per la sincronizzazione dell'ora e abilitare la crittografia dei dati a riposo.
