...
searchtitle: Differenze tra ASA r2 e altri sistemi ONTAP 
---
= Differenze tra ASA r2 e altri sistemi ONTAP
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
I sistemi ASA r2 sono soluzioni hardware e software per ambienti esclusivamente SAN, basate su sistemi all-flash. Si differenziano dagli altri sistemi ONTAP (ASA, AFF e FAS) per la personalità ONTAP, il livello di storage e i protocolli supportati.
...
Su un sistema ASA r2, il software ONTAP è ottimizzato per fornire supporto alle funzionalità SAN essenziali, limitando al contempo la visibilità e la disponibilità di funzionalità e funzioni non correlate alla SAN. Ad esempio, System Manager in esecuzione su un sistema ASA r2 non visualizza le opzioni per creare directory home per i client NAS. Questa versione ottimizzata di ONTAP è identificata come _personalità ASA r2_. ONTAP in esecuzione su sistemi ASA è identificato come _personalità ASA ONTAP_. ONTAP in esecuzione su sistemi AFF e FAS ONTAP è identificato come _personalità unified ONTAP_. Le differenze tra le personalità ONTAP sono indicate nel riferimento ai comandi ONTAP (man page), nelle specifiche REST API e nei messaggi EMS, dove applicabile.
...
Il livello di storage per i sistemi FAS, AFF e ASA utilizza gli aggregati come unità di storage di base. Un aggregato possiede un insieme specifico di dischi disponibili in un sistema storage. L'aggregato alloca spazio sui dischi di sua proprietà ai volumi per LUN e namespace. Con questi sistemi, gli utenti ONTAP possono creare e modificare aggregati, volumi, LUN e namespace.
...
Quando viene creata un'unità di storage (basata su un LUN o su uno spazio dei nomi NVMe), ONTAP crea automaticamente un volume nella zona di disponibilità dello storage appropriata per ospitare l'unità di storage. Il volume appena creato viene automaticamente posizionato all'interno della zona di disponibilità dello storage per garantire prestazioni ottimali e un utilizzo bilanciato della capacità. L'utilizzo della capacità viene bilanciato all'interno della zona di disponibilità dello storage in base alla versione di ONTAP. link:../administer/space-balancing.html["Scopri di più sul bilanciamento della capacità in un cluster ASA r2"^].
...
I nuovi cluster includono automaticamente una macchina virtuale di storage dati predefinita con protocolli SAN abilitati. Le LIF dati IP supportano i protocolli iSCSI e NVMe/TCP e utilizzano la  `default-data-blocks`service policy predefinita.
