...
Quando viene creata un'unità di storage (basata su una LUN o su uno spazio dei nomi NVMe), ONTAP crea automaticamente un volume nella zona di disponibilità dello storage appropriata per ospitarla. Il volume appena creato viene automaticamente posizionato all'interno della zona di disponibilità dello storage per prestazioni ottimali e un utilizzo bilanciato della capacità. L'utilizzo della capacità è bilanciato all'interno della zona di disponibilità dello storage in base alla versione di ONTAP in uso. link:administer/space-balancing.html["Scopri di più sul bilanciamento della capacità in un cluster ASA r2"^].
