...
Esistono diverse differenze tra l'API REST di ASA r2 e l'API REST unificata di ONTAP disponibile con i sistemi FAS, AFF e ASA. Comprendere queste differenze ti aiuterà a utilizzare la documentazione di riferimento dell'API online in modo più efficace.
...
Alcune combinazioni di endpoint e metodi di ASA r2 non supportano tutte le proprietà definite per la personalità ONTAP unificata. Ad esempio, quando usi PATCH con l'endpoint  `/api/storage/volumes/{uuid}`, diverse proprietà non sono supportate con ASA r2, tra cui:
...
OAuth 2.0 è lo standard di settore per i framework di autorizzazione. Viene utilizzato per limitare e controllare l'accesso alle risorse protette in base a token di accesso firmati. Puoi configurare OAuth 2.0 usando System Manager per proteggere le risorse di sistema ASA r2.
