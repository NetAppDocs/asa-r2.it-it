...
[role="lead"]
La clonazione dei dati crea copie delle unità di storage e dei gruppi di coerenza sul tuo sistema ASA r2. Puoi usare le copie per lo sviluppo di applicazioni, test, backup, migrazione dei dati e altre funzioni amministrative.
...
.. Inserisci un nome per il nuovo host; quindi seleziona *FC* o *iSCSI*.
...
.Fasi
. In System Manager, selezionare *protezione > gruppi di coerenza*.
. Passa il mouse sopra il clone del consistency group che vuoi dividere.
