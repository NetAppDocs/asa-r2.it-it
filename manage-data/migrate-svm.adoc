...
[role="lead"]
A partire da ONTAP 9.18.1, puoi migrare senza interruzioni una macchina virtuale di storage (VM) da qualsiasi cluster ASA standard a qualsiasi cluster ASA r2. Migrare da un cluster ASA standard a un cluster ASA r2 ti permette di adottare l'architettura semplificata e ottimizzata dei sistemi ASA r2 per ambienti solo SAN.
