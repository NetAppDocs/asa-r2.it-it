...
[role="lead"]
A partire da ONTAP 9.18.1, puoi migrare senza interruzioni una macchina virtuale di storage (VM) da qualsiasi cluster ASA standard a qualsiasi cluster ASA r2 per adottare l'architettura semplificata e ottimizzata dei sistemi ASA r2 per ambienti solo SAN.


NOTE: Non puoi migrare una storage VM da un ASA MetroCluster a un cluster ASA r2. I sistemi ASA r2 non supportano configurazioni MetroCluster.
...
|===
Per l'elenco più aggiornato dei sistemi ASA e ASA r2, vederelink:https://hwu.netapp.com/["NetApp Hardware Universe"^] .  I sistemi ASA r2 sono elencati in NetApp Hardware Universe come "ASA A-Series/C-Series (New)".
...
----
+
Se devi risolvere eventuali problemi per preparare il tuo cluster ASA alla migrazione, vengono visualizzati il problema e l'azione correttiva. Risolvilo e ripeti il pre-check finché non viene completato con successo.
