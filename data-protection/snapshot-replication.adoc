...
. Seleziona *Avvia cluster remoto*; in questo modo viene generata una passphrase che userai per autenticarti con il cluster remoto.
...
Hai stabilito una connessione peer tra il tuo cluster ASA r2 locale e un cluster remoto. Ora puoi creare una relazione di replica.
...
La policy di replica definisce quando gli aggiornamenti eseguiti sul cluster ASA r2 vengono replicati nel sito remoto. ONTAP include diverse policy di protezione dei dati predefinite che puoi usare per le tue relazioni di replica. Se le policy predefinite non soddisfano le tue esigenze, puoi creare una policy di replica personalizzata.
...
Per una maggiore sicurezza contro gli attacchi ransomware, crea una relazione di replica SnapLock for SnapVault.
...
=== Crea una relazione per replicare gli snapshot in un cluster remoto

Proteggi i tuoi dati replicando gli snapshot su un cluster remoto. Gli snapshot replicati dei tuoi gruppi di coerenza possono essere utilizzati per il ripristino di emergenza o per la migrazione dei dati.
...
.. Cercare e selezionare il gruppo di coerenza di origine.
.. In *destinazione di replica*, seleziona il cluster su cui replicare e poi seleziona la storage VM.


|===
. Selezionare un criterio di replica.
. Per ritardare l'avvio del trasferimento dei dati, seleziona *Impostazioni destinazione* e poi deseleziona *Avvia trasferimento immediatamente*.
+
Il trasferimento iniziale dei dati inizia immediatamente per impostazione predefinita.

. Facoltativamente, per ignorare la pianificazione di trasferimento predefinita, seleziona *Impostazioni destinazione* e poi *Ignora pianificazione di trasferimento*.
+
...
.Quali sono le prossime novità?
Ora che hai creato una policy di replica e una relazione, il trasferimento iniziale dei dati inizia come definito nella policy di replica. Puoi anche testare il failover della replica per verificare che il failover avvenga correttamente se il tuo sistema ASA r2 va offline.



[[create-a-relationship-to-replicate-snapshots-with-snaplock-for-snapvault]]
=== Crea una relazione per replicare gli snapshot con SnapLock for SnapVault

Per proteggerti dagli attacchi ransomware, quando configuri la relazione di replica, puoi abilitare SnapLock per SnapVault selezionando una policy di replica del vault e poi scegliendo di bloccare gli snapshot di destinazione. Gli snapshot protetti da SnapLock per SnapVault non possono essere eliminati accidentalmente o intenzionalmente.


NOTE: I sistemi ASA r2 non supportano la modalità SnapLock Compliance né la modalità SnapLock Enterprise. La creazione di una relazione di replica SnapLock for SnapVault consente solo il blocco delle snapshot di destinazione. Non abilita la modalità SnapLock Compliance o Enterprise.

.Prima di iniziare
Devi link:../secure-data/snaplock.html#initialize-the-snaplock-compliance-clock["inizializza l'SnapLock Compliance Clock"].

.Fasi
. In System Manager, seleziona *Protezione > Gruppi di coerenza* e poi seleziona un gruppo di coerenza.
. Seleziona image:icon_kabob.gif["tre punti blu verticali"], quindi seleziona *Proteggi*.
...
. Seleziona *Impostazioni di destinazione*, quindi seleziona *Blocca le istantanee di destinazione per impedirne l'eliminazione*.
...
.Quali sono le prossime novità?
Ora che hai creato una relazione di replica, il trasferimento iniziale dei dati inizia come definito nella tua policy di replica. Puoi anche testare il failover della replica per verificare che il failover avvenga correttamente se il tuo sistema ASA r2 va offline.
