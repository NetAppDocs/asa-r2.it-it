...
A partire da ONTAP 9.18.1, puoi impostare la percentuale di riserva snapshot e abilitare l'eliminazione automatica degli snapshot quando crei un gruppo di coerenza con nuove unità di storage. La riserva snapshot è la quantità di spazio nell'unità di storage riservata specificamente agli snapshot. Quando la riserva snapshot è impostata insieme all'eliminazione automatica degli snapshot, gli snapshot meno recenti vengono eliminati automaticamente quando lo spazio utilizzato dagli snapshot supera la riserva snapshot. Se la riserva snapshot e l'eliminazione automatica degli snapshot sono abilitate su un gruppo di coerenza padre, vengono abilitate su tutti i gruppi di coerenza figlio esistenti. Se vengono aggiunti nuovi gruppi di coerenza figlio, questi non ereditano le impostazioni di riserva snapshot e di eliminazione degli snapshot del gruppo padre.

link:snapshot-reserve.html["Scopri di più sulla riserva di snapshot sui sistemi di archiviazione ASA r2"].

A partire da ONTAP 9.16.1, quando crei gruppi di coerenza utilizzando nuove unità di storage, puoi configurare fino a cinque gruppi di coerenza secondari. link:manage-hierarchical-consistency-groups.html["Scopri di più sui gruppi di coerenza figlio sui sistemi ASA r2"].
...
====
.Quali sono le prossime novità?
Ora che i tuoi dati sono protetti con gli snapshot, dovresti link:snapshot-replication.html["configurare la replica snapshot"] copiare i tuoi gruppi di coerenza in una posizione geograficamente remota per il backup e il ripristino di emergenza.
