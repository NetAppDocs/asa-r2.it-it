...
summary: Utilizza i gruppi di coerenza per eseguire diverse azioni su più unità di archiviazione contemporaneamente invece di eseguire azioni individualmente su ciascuna unità. In una struttura gerarchica, uno o più gruppi di coerenza sono configurati come figli sotto un gruppo di coerenza padre. 
---
...
[role="lead"]
A partire da ONTAP 9.16.1, puoi usare System Manager per creare gruppi di coerenza gerarchici sul tuo sistema ASA r2. In una struttura gerarchica, uno o più gruppi di coerenza sono configurati come figli sotto un gruppo di coerenza padre. Puoi applicare criteri di snapshot individuali a ciascun gruppo di coerenza figlio e replicare gli snapshot di tutti i gruppi di coerenza figlio in un cluster remoto come un'unica unità replicando il gruppo padre. Questo semplifica la protezione e la gestione dei dati per strutture dati complesse.
...
. Passa il mouse sopra il gruppo di coerenza che vuoi convertire in un gruppo di coerenza padre.
...
Puoi creare ulteriori gruppi di coerenza figlio all'interno del gruppo di coerenza padre. Puoi anche link:snapshot-replication.html["configurare la replica snapshot"] copiare i gruppi di coerenza padre e figlio in una posizione geografica remota per il backup e il disaster recovery.
...
. Passa il mouse sopra il gruppo di coerenza figlio che vuoi scollegare.
