...
| link:../data-protection/manage-consistency-groups.html["Supporto per modifiche geometriche ai gruppi di coerenza nelle relazioni di replicazione"] | I sistemi ASA r2 supportano le modifiche geometriche ai gruppi di coerenza in una relazione SnapMirror active sync o in una relazione di replica asincrona senza eliminare la relazione SnapMirror active sync o interrompere la relazione asincrona. Quando si verifica una modifica geometrica sul gruppo di coerenza primario, la modifica viene replicata sul gruppo di coerenza secondario. 
