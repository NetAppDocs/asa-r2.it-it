...
[role="lead"]
Utilizza System Manager per configurare l'accesso al domain controller di Active Directory, l'autenticazione LDAP e SAML per i tuoi account utente. Crea ruoli per gli account utente per definire funzioni specifiche che gli utenti assegnati a tali ruoli possono eseguire sul tuo cluster.



[[configure-active-directory-domain-controller-access]]
== Configura l'accesso al domain controller di Active Directory

Configura l'accesso del domain controller Active Directory (AD) al tuo cluster o alla tua storage VM così puoi abilitare l'accesso dell'account AD.
...
.Fasi
. In System Manager, selezionare *Cluster > Settings*.
. In *Sicurezza*, accanto a *autenticazione SAML*, seleziona image:icon_gear_white_bg.png["contorno blu di un ingranaggio meccanico"].
...
. Per abilitare l'autenticazione a più fattori (MFA), seleziona image:icon_add.gif["segno più seguito dalla parola aggiungere in lettere blu"]; quindi seleziona un metodo di accesso secondario e un metodo di autenticazione.
