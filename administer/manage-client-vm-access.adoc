...
Durante la configurazione del cluster, viene creata la tua macchina virtuale (VM) di archiviazione dati predefinita. Tutte le nuove unità di archiviazione vengono create all'interno della tua VM di archiviazione dati predefinita, a meno che tu non crei e selezioni una VM di archiviazione diversa. Potresti voler creare una VM di archiviazione aggiuntiva per separare le unità di archiviazione destinate a diverse applicazioni, reparti o clienti. Ad esempio, potresti voler creare una VM di archiviazione per il tuo ambiente di sviluppo e un'altra VM di archiviazione per l'ambiente di produzione, oppure una VM di archiviazione per il reparto finanziario e un'altra VM di archiviazione per il reparto marketing.
...
In caso di guasto di un componente, un LIF può eseguire un failover o essere migrato a una porta fisica diversa, così la comunicazione di rete non viene interrotta. A partire da ONTAP 9.19.1, il failover automatico dei LIF è supportato per impostazione predefinita per gli IP LIF. Quando il failover automatico dei LIF è abilitato, se si verifica un failover dello storage, il LIF viene automaticamente migrato dal suo nodo o porta home al nodo o porta del partner HA e poi di nuovo al nodo o porta home una volta completato il failover. Oppure, se la porta diventa non integra, il LIF viene automaticamente migrato a una porta integra nel suo nodo home attuale e poi di nuovo alla sua porta originale una volta che la porta è di nuovo integra.

In ONTAP 9.18.1 e versioni precedenti, il failover automatico delle LIF non è supportato per le LIF IP con traffico sia iSCSI che NVMe/TCP. Quando crei LIF solo iSCSI, il failover automatico delle LIF è abilitato per impostazione predefinita, a meno che la storage VM non abbia già LIF iSCSI per cui il failover automatico non è abilitato. Se sulla storage VM sono presenti LIF iSCSI per cui il failover automatico non è abilitato, anche le nuove LIF che crei non saranno abilitate per il failover automatico. In questi casi, devi prima eliminare le LIF iSCSI esistenti e poi ricrearle con il failover automatico abilitato. Scopri come link:https://docs.netapp.com/us-en/ontap/san-admin/delete-lif-san-environment-task.html["eliminare un LIF in un ambiente SAN"^].
...
In ONTAP 9.18.1 e versioni precedenti, il failover automatico delle LIF è abilitato per impostazione predefinita quando crei LIF solo iSCSI, se anche tutte le altre LIF iSCSI sulla VM di storage sono abilitate per il failover automatico. Se sulla VM di storage sono presenti LIF iSCSI non abilitate per il failover automatico, anche le LIF appena create non saranno abilitate per il failover automatico.

.Prima di iniziare
È necessario aver creato un criterio di servizio LIF personalizzato solo iSCSI.

.A proposito di questa attività
Devi usare l'interfaccia a riga di comando (CLI) di ONTAP per creare un LIF solo iSCSI.
