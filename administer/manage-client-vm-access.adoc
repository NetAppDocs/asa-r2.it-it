...
.Quali sono le prossime novità?
Se hai creato una LIF IP per iSCSI, puoi usare l'interfaccia a riga di comando (CLI) di ONTAP per abilitare il failover automatico di LIF iSCSI.
...
Devi usare l'interfaccia a riga di comando (CLI) di ONTAP per abilitare il failover automatico di iSCSI LIF.
