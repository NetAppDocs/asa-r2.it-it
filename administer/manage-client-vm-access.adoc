...
Se sulla VM di archiviazione sono presenti LIF iSCSI non abilitati per il failover LIF automatico, anche i LIF appena creati non saranno abilitati per il failover LIF automatico.  Se il failover LIF automatico non è abilitato e si verifica un evento di failover, i LIF iSCSI non verranno migrati.
