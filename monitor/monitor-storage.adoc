...
Se il tuo cluster sta per esaurire lo spazio disponibile o non ha la capacità per soddisfare la domanda futura, link:../administer/increase-storage-capacity.html["aggiungere nuove unità"] al tuo sistema ASA r2 per aumentare la capacità di storage.
...
+
In *Panoramica*, vengono visualizzati la capacità della zona di disponibilità dello storage, la quantità di spazio utilizzato e il rapporto di riduzione dei dati.
...
.Quali sono le prossime novità?
Se la disponibilità dello storage nella tua zona di disponibilità sta per esaurirsi, link:../manage-data/move-storage-units.html["spostare le unità di archiviazione"] in un'altra zona di disponibilità dello storage per bilanciare l'utilizzo dello storage nel cluster, elimina le snapshot per liberare spazio oppure aggiungi dischi aggiuntivi ai tuoi nodi per aumentare la capacità.
