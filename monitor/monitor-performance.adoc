...
[role="lead"]
Utilizza ONTAP System Manager per monitorare le prestazioni complessive del tuo cluster e delle singole unità di storage, inclusi latenza, IOPS e throughput, per periodi di tempo che vanno da un'ora a un anno.
...
Utilizza le metriche delle performance dell'unità di storage per determinare l'impatto di applicazioni specifiche su latenza, IOPS e throughput.
...
.Quali sono le prossime novità?
Utilizza le metriche delle prestazioni delle tue unità di storage per analizzare se devi modificare le policy QoS assegnate alle tue unità di storage per ridurre la latenza e massimizzare IOPS e throughput.
